    ...
    <div className={`min-h-screen bg-gradient-to-b ${visuals.bgGradient} flex flex-col text-white relative`}>
      ...
      <AnimatePresence>
        {collectAnim && (
          ...
          >
            {getEmojiSprite(collectAnim) ? (
              <img src={getEmojiSprite(collectAnim)!} alt="" className="w-12 h-12 object-contain drop-shadow-xl" draggable={false} />
            ) : (
              <span className="text-5xl">{collectAnim}</span>
            )}
          </motion.div>
        )}
      </AnimatePresence>
