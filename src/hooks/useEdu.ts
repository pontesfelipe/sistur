...
export function useEduTrackMutations() {
  ...
  const createTrackWithTrainings = useMutation({
    mutationFn: async ({ 
      ...
      // Optionally auto-generate per-pillar final exams for the track.
      // Failures here are non-fatal: the track exists, we just warn.
      let examsCreated = 0;
      if (autoGenerateExams && trainingIds.length > 0) {
        const { data: examResults, error: examError } = await (supabase.rpc as any)(
          'generate_track_exam_rulesets',
          {
            p_track_id: newTrack.id,
            p_question_count: 20,
            p_min_score_pct: 70,
            p_time_limit_minutes: 60,
            p_max_attempts: 2,
            p_overwrite: false,
          }
        );
        if (examError) {
          console.error('Auto-generate exams failed:', examError);
        } else {
          examsCreated = (examResults || []).filter((r: any) => r.created).length;
        }
      }

      return { ...newTrack, _examsCreated: examsCreated, _autoGenerated: autoGenerateExams };
    },
    onSuccess: (result: any) => {
      queryClient.invalidateQueries({ queryKey: ['edu-tracks'] });
      queryClient.invalidateQueries({ queryKey: ['edu-track-trainings'] });
      queryClient.invalidateQueries({ queryKey: ['track-exams', result.id] });
      if (result._autoGenerated && result._examsCreated > 0) {
        toast.success(
          `Trilha criada com ${result._examsCreated} prova${result._examsCreated > 1 ? 's' : ''} final${result._examsCreated > 1 ? 'is' : ''} gerada${result._examsCreated > 1 ? 's' : ''}`
        );
      } else {
