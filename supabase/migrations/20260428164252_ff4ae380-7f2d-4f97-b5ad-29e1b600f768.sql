...
BEGIN
  ...
  SELECT raw_value, reference_year INTO v_pib_mun, v_pib_mun_year
  FROM public.external_indicator_values
  WHERE municipality_ibge_code = p_ibge_code AND indicator_code = 'igma_pib_per_capita' AND validated = true
  ORDER BY reference_year DESC NULLS LAST LIMIT 1;

  IF v_pib_mun IS NOT NULL AND v_pib_mun > 0 THEN
    SELECT value, reference_year INTO v_pib_br, v_pib_br_year
    FROM public.national_reference_values
    WHERE indicator_code = 'igma_pib_per_capita'
      AND reference_year <= COALESCE(v_pib_mun_year, 9999)
