...
BEGIN
  ...
  -- Batch update all grades in a single statement
  UPDATE public.exam_answers AS a
  SET awarded_points = LEAST(GREATEST(0, (g.val->>'points')::numeric), 100.0 / GREATEST(_total_answers, 1)),
      is_correct = ((g.val->>'points')::numeric > 0),
      grader_comment = NULLIF(g.val->>'comment', '')
  FROM jsonb_array_elements(_grades) AS g(val)
  WHERE a.attempt_id = _attempt_id
    AND a.quiz_id = (g.val->>'quiz_id')::uuid;
